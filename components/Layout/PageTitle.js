  ...
  return (
    <div
      className={ClassCombiner(
        "flex justify-between items-center m-6",
        className
      )}
    >
      <div className="text-3xl font-bold p-2 w-full flex items-center gap-4">
        {allowBack && (
          <div className="select-none cursor-pointer" onClick={handleClickBack}>
            {IconArrowBackCircle}
          </div>
        )}
      ...
      {action && <div className="w-full text-right">{action}</div>}
    </div>
  );
};
